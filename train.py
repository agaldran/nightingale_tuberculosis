...
def get_args_parser():
    ...
    parser.add_argument('--bce_ls', type=float, default=0.1, help='label smoothing for bce loss')
    parser.add_argument('--batch_size', type=int, default=8, help=' batch size')
    parser.add_argument('--acc_grad', type=int, default=1, help='gradient accumulation')
    parser.add_argument('--ovsmpl', type=str2bool, nargs='?', const=True, default=True, help='oversmapling (sqrt)')
    parser.add_argument('--tr_aug', type=str2bool, nargs='?', const=True, default=False, help='trivial augment')    
...
def train_one_epoch(model, loader, acc_grad, loss_fn, optimizer, scheduler):
    model.train()
    device = 'cuda' if next(model.parameters()).is_cuda else 'cpu'
    ...
    with trange(len(loader)) as t:
        ...
        for (i_batch, batch_data) in enumerate(loader):
            ...
            loss = loss / acc_grad
            loss.backward()
            if ((i_batch + 1) % acc_grad == 0) or (i_batch + 1 == len(tr_loader)):
                # Update Optimizer
                optimizer.step()
                optimizer.zero_grad()
                scheduler.step()
...
def get_eval_string(tr_info, epoch, finished=False, vl_interval=1):
    ...
    return s
def train_model(model, optimizer, acc_grad, loss_fn, tr_loader, ovft_loader, vl_loader, ovsmpl, scheduler, metric, n_epochs, vl_interval, save_path):
    ...
    for epoch in range(n_epochs):
        ...
        train_one_epoch(model, loader, acc_grad, loss_fn, optimizer, scheduler)
...
if __name__ == '__main__':
    ...
    lr, bs, acc_grad, ovsmpl = args.lr, args.batch_size, args.acc_grad, args.ovsmpl
    ...
    n_classes = 1
    model = get_model(args.model_name, n_classes=n_classes)

    print('* Creating Dataloaders, batch size = {}, acc_grad = {}, workers = {}'.format(bs, acc_grad, nw))

    tr_loader, ovft_loader, vl_loader = get_train_val_loaders(csv_path_tr, bs, im_size, num_workers=nw)
    ...
    if args.cyclical_lr:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=vl_interval*len(tr_loader)//acc_grad, eta_min=0)
    else:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=n_epochs*len(tr_loader)/acc_grad, eta_min=0)
    ...
    start = time.time()
    tr_info = train_model(model, optimizer, acc_grad, loss_fn, tr_loader, ovft_loader, vl_loader, ovsmpl, scheduler, metric, n_epochs, vl_interval, save_path)
    end = time.time()
